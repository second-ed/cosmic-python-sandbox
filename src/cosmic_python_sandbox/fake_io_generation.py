...
def generate_fake_io(
    ...
    read_ext_src = (
        ...
        f"{tab * 2}self.log.append(('external_read', path))",
...
def get_db_funcs(tab: str) -> tuple[str]:
    ...
    read_func = (
        ...
        f"{tab * 2}self.log.append(('read', path))",
    ...
    write_func = (
        ...
        f"{tab * 2}self.log.append(('write', path))",
...
def get_standard_ops(tab):
    ...
    remove_func = (
        f"{tab}def remove(self, path: str, new_path: str) -> bool:",
        f"{tab * 2}self.log.append(('remove', path))",
        f"{tab * 2}self.db.pop(path)",
        f"{tab * 2}return True",
    )
    return size_func, copy_func, move_func, remove_func
